...
function KIBRA.Natives.TableUpdate(vPlayer)
    ...
    xPlayer.removeItem = function(item, amount, slot, metadata)
        if framework == "ESX" then
            vPlayer.removeInventoryItem(item, amount, metadata)
        else
            vPlayer.Functions.RemoveItem(item, amount, slot, metadata)
        end
    end
