...
RegisterNetEvent('KIBRA:Client:TriggerCallback', function(name, ...)
    if KIBRA.ServerCallbacks[name] then
        KIBRA.ServerCallbacks[name](...)
        KIBRA.ServerCallbacks[name] = nil
    end
end)

KIBRA.DrawMarker = function(type, coord, distance)
    local getDistance = #(GetEntityCoords(PlayerPedId()) - coord)
...
KIBRA.GetServerPlayers = function()
    local AllPlayers = {}
    local ActivePlayers = GetActivePlayers()
    for k,v in pairs(ActivePlayers) do
        local xPlayer = ActivePlayers[k]
        local GetPed = GetPlayerPed(xPlayer)
        if DoesEntityExist(GetPed) then
            AllPlayers[#AllPlayers+1] = xPlayer
        end
    end    
    return AllPlayers
end
...
KIBRA.Round = function(value, numDecimalPlaces)
    if not numDecimalPlaces then return math.floor(value + 0.5) end
    local power = 10 ^ numDecimalPlaces
    return math.floor((value * power) + 0.5) / (power)
end

KIBRA.GetPlate = function(vehicle)
    if vehicle == 0 then return end
    return KIBRA.Trim(GetVehicleNumberPlateText(vehicle))
end

KIBRA.GetClosestPlayer = function(coords)
    ...
    local closestPlayers = KIBRA.GetPlayersFromCoords(coords)
...
KIBRA.GetPlayersFromCoords = function(coords, distance)
...
KIBRA.GetVehicleProps = function(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        return {
            ...
            plate = KIBRA.GetPlate(vehicle),
            plateIndex = GetVehicleNumberPlateTextIndex(vehicle),
            bodyHealth = KIBRA.Round(GetVehicleBodyHealth(vehicle), 0.1),
            engineHealth = KIBRA.Round(GetVehicleEngineHealth(vehicle), 0.1),
            tankHealth = KIBRA.Round(GetVehiclePetrolTankHealth(vehicle), 0.1),
            fuelLevel = KIBRA.Round(GetVehicleFuelLevel(vehicle), 0.1),
            dirtLevel = KIBRA.Round(GetVehicleDirtLevel(vehicle), 0.1),
            oilLevel = KIBRA.Round(GetVehicleOilLevel(vehicle), 0.1),
            color1 = colorPrimary,
...
KIBRA.SetVehicleProps = function(vehicle, props)
    if DoesEntityExist(vehicle) then
